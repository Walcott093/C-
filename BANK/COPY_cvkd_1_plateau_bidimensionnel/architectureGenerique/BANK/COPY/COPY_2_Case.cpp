...
bool Case::ajouterPion(Pion p) {
  if(nbPions <= nbPionsMax)
    ...
    cout << "[Case.cpp] Erreur : Impossible de rajouter le pion. Case remplie." << endl;
    return false;
  }
  return true;
}
bool Case::ajouterPions(Pion t[], int nb) {
  bool ret = false;
  if(nbPions + nb > nbPionsMax)
    return ret;
  else
    for(int i=0 ; i<nb ; i++)
      ret = ajouterPion(t[i]);
...
bool Case::retirerPion(Pion p) {
  if(nbPions == 0) {
    cout << "[Case.cpp] Erreur : Impossible de retirer le pion. Case vide." << endl;
    return false;
  }
  else
    // si on retire il faut decaler tous les autres pions... peut-etre une liste ?
}
